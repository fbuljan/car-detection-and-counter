...
def load_finetuned_fasterrcnn_model(weights_path, num_classes=2):
...
def main():
    parser = argparse.ArgumentParser(description="Detect and count cars using fine-tuned Faster R-CNN.")
    parser.add_argument('--weights', type=str, default='models/fasterrcnn_car.pth',
                        help='Path to fine-tuned Faster R-CNN weights')
    parser.add_argument('--output', type=str, default='count_cars_outputs/count_cars_outputs_fast_r_cnn_fine_tuned',
                        help='Output folder')
    args = parser.parse_args()

    print(f"[INFO] Učitavanje fine-tuned Faster R-CNN modela iz {args.weights} ...")
    model = load_finetuned_fasterrcnn_model(args.weights, num_classes=2)
